#!/bin/sh
#
# 2017-09-19 - initial script by Bretton
# - depends scrapeapnic.rb (nokogiri script to pull data)
# 2017-09-21 - added to user wider directory
# 2017-09-22 - set a base directory variable so cron doesn't complain
# 2017-11-03 - installed on wider.isoc.org.za vhost
#
# set base
base="/home/wider/scrapewider"
...
$base/scrapeapnic.rb > $base/output/output1.txt
...
# replace header by new header
/bin/sed -e '1 s/.*/asnrank,asn,asname,country_code,users,gdpxusers,percentageofcountry,percentageofinternet,v6users,percentageofas,percentageofcountryv6total,percentageofinternetv6total,samples/g' $base/output/output9.txt > $base/output/output10.txt

# copy output10.txt to CSV file we want to use
/bin/cp $base/output/output10.txt $base/csvout/wider.$date.csv

# insert data in the main table in the database
#csvsql -v --db postgresql://wider:'XXXXX'@localhost:5432/widerstore --tables main $base/csvout/wider.$date.csv --insert --overwrite --no-create

# turn CSV file into json using node.js tool csv2json
# installed via npm to /usr/local/bin/csv2json
/usr/local/bin/csv2json $base/csvout/wider.$date.csv $base/jsonout/wider.$date.json
